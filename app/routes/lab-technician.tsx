...
import { adminNavLinks, technicianNavLinks } from "~/data/nav-links";
      ...
      >
        <div className="flex flex-col gap-10">
          ...
          <div className="flex flex-col gap-1 p-2">
            {technicianNavLinks.map(
              (link: AdminNavLinkInterface, index: number) => {
                const trimmedPathname = pathname.startsWith("/lab-technician")
                  ? pathname.slice(1)
                  : pathname;
                const trimmedLinkPath = link.path.startsWith("/lab-technician")
                  ? link.path.slice(1)
                  : link.path;

                const isActive =
                  (link.path === "/lab-technician" &&
                ...
                return (
                  <Link
                    key={index}
                    to={link.path}
                    className={`flex items-center gap-2 p-2 text-white hover:text-blue-300 transition-all duration-300 font-nunito text-sm rounded-xl ${
                      isActive ? "bg-indigo-500" : ""
                    }`}
                  >
                    {link.icon}
...
export const loader: LoaderFunction = async ({ request }) => {
  ...
  await userController.checkUserRole("lab-technician");
