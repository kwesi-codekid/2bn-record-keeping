...
import { adminNavLinks, staffNavLinks } from "~/data/nav-links";
    ...
    >
      ...
      >
        ...
        <div className="flex flex-col gap-2 p-2">
          {staffNavLinks.map((link: AdminNavLinkInterface, index: number) => {
            return (
              <NavLink
                key={index}
                to={link.path}
      ...
      >
        <div className="flex flex-col gap-10">
          <div
            className={`bg-blue-600 rounded-2xl flex flex-col shadow-2xl shadow-blue-800/30 ${
              !isDesktopExpanded
          ...
          >
            <img
              src={logo}
              alt="logo"
              className={`${isDesktopExpanded ? "w-24" : "w-8"}`}
            />
          ...
          <div className="flex flex-col gap-1 p-2">
            {staffNavLinks.map((link: AdminNavLinkInterface, index: number) => {
              const trimmedPathname = pathname.startsWith("/admin")
                ? pathname.slice(1)
                : pathname;
              const trimmedLinkPath = link.path.startsWith("/admin")
                ? link.path.slice(1)
                : link.path;

              // const isActive =
              //   (link.path === "/admin" && pathname === "/admin") ||
              //   (link.path !== "/admin" &&
              //     (pathname === link.path ||
        ...
        <div className="flex flex-col gap-2">
          ...
          <header className="bg-indigo-600 flex items-center justify-between py-1 px-4 md:pl-2 rounded-2xl">
            <div className="flex items-center gap-3 ">
              ...
              <NavHeader navLinks={staffNavLinks} basePath="/staff" />
            </div>
...
export const loader: LoaderFunction = async ({ request }) => {
  ...
  await userController.checkUserRole("staff");
