...
import {
  generateCompletedTreatmentNotification,
  generateHODNotification,
...
export default class VisitController {
  ...
  public getReferralFeedback = async (
    id: string
  ): Promise<VisitInterface | any> => {
    try {
      const feedback = await ReferralFeedback.findOne({
        visit: id,
      });
  ...
  public addVisitConsultation = async ({
    ...
    try {
      ...
      const currentUser = await userController.getUser();
      // Find the visit and check if the visit record has an existing vitals record
      const visit = await Visit.findOne({
        _id: visitId,
        "visitRecords._id": visitSubId,
      ...
      const staff = await User.findById(updatedVisit?.staff);
      await sendEmail({
        to: `${staff?.email}`,
        subject: "Treatment Completed",
        html: generateCompletedTreatmentNotification({
          staffName: staff?.firstName + " " + staff?.lastName,
          reviewDate,
          treatmentCompletionDate: moment(new Date().toDateString()).format(
            "DD-MM-YYYY"
          ),
          medicalOfficer: currentUser?.firstName + " " + currentUser?.lastName,
          workStatus: work_status,
          excuseDutyDuration: excuseDutyDuration,
        }),
      }).then((response) => {
      ...
      return {
        status: "error",
        code: 200,
        message: "Error added consultaion",
        errors: [],
      };
    }
  };
  ...
  public addVisitReferralFeedback = async ({
    ...
    try {
      const existingRec = await ReferralFeedback.findOne({
        visit: visitId,
      });

      if (existingRec) {
        await ReferralFeedback.findOneAndUpdate(
          {
            visit: visitId,
          },
          {
        ...
        const updatedVisit = await Visit.findOneAndUpdate(
          { _id: visitId, "visitRecords._id": visitSubId },
          { $set: { "visitRecords.$.referralFeedback": feedback?._id } },
          { new: true }
        ).exec();
      ...
      console.error("Error updating visit referral feedback:", error);

      return {
        code: 400,
        status: "error",
        messagee: "Error updating visit referral feedback",
        errors: [],
      };
    }
  };
      ...
      labTehnicians.forEach(async (labTech) => {
        await sendEmail({
          ...
          html: generateLabRequestNotification({
            ...
            requestingMedicalOfficer: (user?.firstName +
              " " +
      ...
      if (labRequest?.requestedBy?.email) {
        await sendEmail({
          ...
          html: generateLabResultsNotification({
            staffName:
              labRequest.staff.firstName + " " + labRequest.staff.lastName,
            labRequestDate: labRequest?.createdAt.toDateString() as string,
            resultsDate: new Date().toDateString() as string,
            requestingMedicalOfficer: (labRequest?.requestedBy?.firstName +
              " " +
              labRequest?.requestedBy?.lastName) as string,
            labsRequested: labRequest.labTests.map(
              (lab) => `<li> ${lab.test} - ${lab.result} </li>`
            ),
          }),
        }).then((response) => {
          console.log(response);
