...
export default class MedicalRequestController {
            ...
            heads.forEach(async (head) => {
              await sendEmail({
                ...
                html: generateNewTreatmentNotification({
                  staffName: userFor?.firstName + " " + userFor?.lastName,
                  managerName: (head.firstName + " " + head.lastName) as string,
                  requestDate: moment(new Date().toDateString()).format(
                    "DD-MM-YYYY"
                  ),
                  initialComplaint,
                }),
          ...
          if (heads.length > 0) {
            heads.forEach(async (head) => {
              await sendEmail({
                ...
                html: generateReviewNotificationEmail({
                  staffName: userFor?.firstName + " " + userFor?.lastName,
                  managerName: head.firstName + " " + head.lastName,
                  requestDate: moment(new Date().toDateString()).format(
                    "DD-MM-YYYY"
                  ),
                }),
      ...
      if (updatedRequest?.requestedFor?.email) {
        ...
        await sendEmail({
          ...
          html: generateApprovalEmail({
            staffName: ((updatedRequest?.requestedFor?.firstName as string) +
              " " +
              updatedRequest?.requestedFor?.lastName) as string,
            approvalDate: moment(new Date().toDateString()).format(
              "DD-MM-YYYY"
            ),
        ...
        await sendEmail({
          ...
          html: generateMedicalOfficerAprrovedRequestNotification({
            staffName:
              updatedRequest?.requestedFor?.firstName +
              " " +
              updatedRequest?.requestedFor?.lastName,
            approvalDate: moment(new Date().toDateString()).format(
              "DD-MM-YYYY"
            ),
            approvedBy: user.firstName + " " + user.lastName,
      ...
      if (updatedRequest?.requestedFor?.email) {
        await sendEmail({
          ...
          html: generateDeclinedRequestNotification({
            ...
            requestDate: moment(
              updatedRequest?.createdAt.toDateString()
            ).format("DD-MM-YYYY"),
          }),
        }).then((response) => {
          console.log(response);
